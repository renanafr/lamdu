...
module Lamdu.Sugar.Types
  ...
  -- Input types:
  , InputPayloadP(..), ipEntityId, ipInferred, ipStored, ipData
...
data WrapAction m
  ...
  | WrappedAlready EntityId -- I'm an arg of apply-of-hole (EntityId of apply), no need to wrap
  | WrapNotAllowed -- I'm already wrapped or a tag or a hole
  | WrapAction (T m (Guid, EntityId)) -- Wrap me!

data SetToHole m
  = SetToHole (T m EntityId)
